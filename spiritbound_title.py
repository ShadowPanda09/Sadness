...
def main_menu():
    ...
    # --- Constants for "Spirit"/"Bound" animation ---
    TITLE_ANIM_START_TIME = 10   # seconds after aura fade starts
    TITLE_ANIM_DURATION = 3    # duration for words to move to final Y position
    ...
    # --- Text intro ---
    text_list = ["MethodicGnat", "ShadowPanda09", "Present"]
    for text_str in text_list:
        ...
        while intro_time < display_duration:
            ...
            if intro_time < fade_duration:
                ...
                fade_alpha = int(255*max(0, (display_duration-intro_time)/fade_duration))
            font_size = 60 if text_str != "Presents" else 48
    ...
    # --- Aura fade-in with pre-title animation ---
    menu_visible = False
    aura_time = 0
    title_anim_done = False
    title_anim_time = 0.0
    ...
    while not menu_visible:
        ...
        # --- Pre-main-menu title animation ---
        if music_pos >= TITLE_ANIM_START_TIME and not title_anim_done:
            progress = min(1.0, (title_anim_time - TITLE_ANIM_START_TIME)/TITLE_ANIM_DURATION)
            start_y = HEIGHT // 2
            final_y = HEIGHT // 4
            y_offset = start_y - (start_y - final_y) * progress

            font_spirit = pygame.font.Font(None, 72)
            font_bound = pygame.font.Font(None, 72)
            spirit_surf = font_spirit.render("Spirit", True, WHITE)
            bound_surf = font_bound.render("Bound", True, WHITE)

            # "Bound" fades in halfway
            bound_alpha = 255 if progress > 0.5 else int(255*(progress*2))
            spirit_surf.set_alpha(255)
            bound_surf.set_alpha(bound_alpha)
            ...
            if progress >= 1.0:
                title_anim_done = True

        # Show main menu once music reaches HALF_TIME
        if music_pos >= HALF_TIME:
            menu_visible = True
            flash_alpha = 255
            flash_done = False
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
            elif event.type == pygame.KEYDOWN and menu_visible:
                if event.key in (pygame.K_UP, pygame.K_w):
                    selected_index = (selected_index - 1) % len(menu_options)
                elif event.key in (pygame.K_DOWN, pygame.K_s):
                    selected_index = (selected_index + 1) % len(menu_options)
