try:
    # If run as part of a package
    from . import spiritbound_game
except ImportError:
    # If run directly
...
import math
import os
from levels.boss_level import BossLevel
...
class Game:
    def __init__(self):
        ...
        self.knight_kills = 0            # total kills by player
        self.PASSING_KILLS = 20          # number of kills needed to trigger boss
        self.transition_to_boss = False
        self.transition_start = None
        self.TRANSITION_DURATION = 1000  # 1 second delay before switching
        self.boss_launched = False

        # Transition visual overlay
        self.boss_transition = False
        self.boss_transition_start = None
        self.BOSS_TRANSITION_DURATION = 1200  # ms
    ...
    def update(self, dt):
        ...
        # ---------------- Boss transition ----------------
        if self.boss_transition:
            elapsed = pygame.time.get_ticks() - self.boss_transition_start
            t = min(1.0, elapsed / self.BOSS_TRANSITION_DURATION)  # 0 → 1
            # freeze gameplay during transition
            if elapsed >= self.BOSS_TRANSITION_DURATION:
                self.launch_boss_level()
                return
            else:
                return  # skip rest of update while transition
        ...
        if self.player.swinging:
            ...
            for knight in list(self.knights):
                ...
                if math.hypot(kx - tip.x, ky - tip.y) <= PLAYER_SWORD_HIT_RADIUS:
                    ...
                    if knight.health <= 0:
                        self.player.health = min(PLAYER_MAX_HEALTH, self.player.health + max(1, KNIGHT_DAMAGE // 10))
                        try: self.knights.remove(knight)
                        except ValueError: pass
                        self.knight_kills += 1
                        if self.knight_kills >= self.PASSING_KILLS and not self.boss_transition:
                            self.boss_transition = True
                            self.boss_transition_start = pygame.time.get_ticks()
        ...
        if self.player.juggernaut_active:
            elapsed = now - self.player.juggernaut_start
            if elapsed >= self.player.juggernaut_duration:
                ...
                for k in self.knights:
                    ...
                    dist = vec.length()
                    if dist <= self.player.juggernaut_radius and dist > 0:
                        k.pos += vec.normalize() * 8
    ...
    def launch_boss_level(self):
        try:
            pygame.mixer.stop()
            pygame.display.quit()
            BossLevel().run()  # Pass player state if needed
        except Exception as e:
            print("Error launching boss level:", e)
            self.running = False
    ...
    def draw(self):
        ...
        # boss transition overlay (glowing neon burst to dark)
        if getattr(self, 'boss_transition', False):
            elapsed = pygame.time.get_ticks() - self.boss_transition_start
            t = min(1.0, elapsed / self.BOSS_TRANSITION_DURATION)
            ...
            # draw multiple expanding neon circles
            for i in range(50):
                color = neon_colors[i % len(neon_colors)]
                radius = int((self.window_width + self.window_height) / 4 * t * (0.5 + 0.5 * (i % 5) / 5))
                alpha = int(200 * (1 - t) * (0.5 + 0.5 * (i % 5) / 5))
                pygame.draw.circle(overlay, (*color, alpha), (int(center_x), int(center_y)), radius, width=6)

            # fade to dark overlay
            dark_alpha = int(255 * t)
            dark_overlay = pygame.Surface((self.window_width, self.window_height))
            dark_overlay.fill((0, 0, 0))
            dark_overlay.set_alpha(dark_alpha)
